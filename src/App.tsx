...
export default function App() {
  ...
  const overloaded = providers.filter((p) => {
    const c = counts[p.id];
    return c && (c.weekDays > p.targetWeekDays || c.weekendDays > p.targetWeekendDays || c.weekNights > p.targetWeekNights || c.weekendNights > p.targetWeekendNights);
  });
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 flex items-center gap-3">
                <div className="p-2.5 bg-gradient-to-b from-blue-500 to-blue-700 shadow-inner rounded-xl"><CalendarIcon className="w-8 h-8 text-white drop-shadow-sm" /></div>
            ...
            <div className="flex items-center gap-3 shrink-0 flex-wrap">
              ...
              <button onClick={clearAssignments} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-slate-600 bg-white/60 backdrop-blur-md border border-slate-200 hover:bg-slate-50 rounded-xl shadow-sm transition-all"><XCircle className="w-4 h-4" />Clear</button>
